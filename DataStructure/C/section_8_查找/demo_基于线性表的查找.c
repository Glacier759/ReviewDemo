 ...
 * =====================================================================================
 *
 *       Filename:  demo_基于线性表的查找.c
 *
 *    Description:  基于线性表的查找
 *
 *        Version:  1.0
 ...
 * 顺序查找
 * */
int		seqSearch( SeqList *L, ElemType x ) {
 ...
 * 折半查找
 * */
int		binSearch( SeqList *L, ElemType x ) {
	int		i, j, m;
	for ( i = 0, j = L->length - 1, m = (i + j) / 2; L->elem[m] != x; m = (i + j) / 2 ) {
		if ( L->elem[m] > x ) {
			j = m - 1;
		}
		else {
...
int insert_SeqList( SeqList *L, int i, ElemType x ) {
	...
	if ( i < 0 || i > L->length + 1 ) {		//检查插入位置的正确性
		printf("位置错");
		return FALSE;
	}
...
int delete_SeqList( SeqList *L, int i ) {
	...
	int j;
	if ( i < 0 || i > L->length ) {				//检查空表及删除位置的合法性
		printf("不存在第i个元素");
		return FALSE;
	}
...
int main(int argc, char *argv[])
{
	...
	for ( int i = 0; i < count; i ++ ) {
		printf("input number: ");
		scanf("%d", &number);
		insert_SeqList( L, i, number );
	}
	print_SeqList( L );

	printf("input number what you want: ");
	scanf("%d", &number);
	//printf("index = %d\n", seqSearch( L, number ));
	printf("index = %d\n", binSearch( L, number ));
	return EXIT_SUCCESS;
