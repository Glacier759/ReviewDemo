...
public class Consumer extends Thread {
    ...
    public static int max_consumer_number = 4;
        ...
        synchronized (Consumer.class) {
            ...
            while (baoZi == null) {
                try {
                    ...
                    logger.info( this.consumer_name + " 要求上包子! 劳资TM等 " + sleep_time_continue_count + " 年了！！" );
                    Thread.sleep(1000);
                }catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            /******************************************************************************************/
            if ( baoZi != null ) {
                logger.info(this.consumer_name + " - 吃掉了一个包子 【" + baoZi.getIndex() + " 】篮子里还有【" + this.basket.baoZiList.size() + "】个");
                sleep_time_continue_count = 0;
                consumer_all_number ++;
            }
    ...
    public void run() {
        while(true) {
            ...
            if ( baoZi == null ) {
                logger.info("等了 " + max_sleep_time_continue + " 年了还TM不给上包子！？ 不吃了！");
                break;
            }
            try {
                this.count ++;
                ...
                if ( this.count == this.max_consumer_number ) {
                    logger.info( this.consumer_name + " - 今天吃了 " + this.max_consumer_number + " 个包子,吃饱了遛弯儿去~" );
                    break;
                }
